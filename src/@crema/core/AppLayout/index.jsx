...
const AppLayout = () => {
  ...
  // Determine if the current route is part of ecomadminconfig
  const isEcommAdminRoute = ecomadminconfig.some(route =>
    window.location.pathname.startsWith(route.path)
  );


  // Use MiniSidebar layout for ecomadminconfig routes
  const AppLayout = isEcommAdminRoute ? MiniSidebar : Layouts[navStyle];
